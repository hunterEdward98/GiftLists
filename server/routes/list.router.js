...
router.put("/", rejectUnauthenticated, (req, res) => {
  if (
    req.user.auth_level < 3 ||
    req.user.auth_level < req.body.auth ||
    req.user.auth_level < req.body.old_auth ||
    (req.user.org_id != req.body.org_id && req.user.auth_level < 6)
  ) {
    res.sendStatus(403);
  } else {
    const body = req.body;
    ...
    pool
      .query(queryText, [body.user, body.auth, body.id])
...
router.delete("/:id/:auth/:org_id", rejectUnauthenticated, (req, res) => {
  if (
    (req.user.auth_level < 3 && req.user.auth_level <= req.params.auth_level) ||
    (req.user.org_id != req.body.org_id && req.user.auth_level < 6)
  ) {
    res.sendStatus(403);
  } else {
